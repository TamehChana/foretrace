...
/**
 * Shows the workspace UUID with a copy button (for CLI `FORETRACE_ORGANIZATION_ID`, support, docs).
 */
export function OrganizationIdCopyRow({
  organizationId,
  className = '',
}: {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 text-[10px] leading-snug text-zinc-500 dark:text-zinc-400">
        Use as <span className="font-mono">FORETRACE_ORGANIZATION_ID</span> for the CLI; same value as{' '}
        <span className="font-mono">?org=</span> in the URL.
      </p>
    </div>
  );
}
